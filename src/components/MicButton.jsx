...
export default function MicButton() {
  ...
  return (
    <>
      <button
        onClick={handleClick}
        className={`fixed bottom-20 right-4 z-50 w-14 h-14 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 ${listening
      ...
      {showResult && (transcript || answer) && (
        <div className="fixed bottom-36 right-4 left-4 z-50 bg-white rounded-2xl shadow-xl border border-gray-100 p-4">
          ...
          {answer && (
            <p className={`text-sm font-medium leading-relaxed ${intent === 'unknown' ? 'text-red-500' : 'text-green-700'
              }`}>
              {answer}
            </p>
          )}
